import { Facebook, Instagram, Linkedin, Youtube } from "lucide-react";

const YEAR = new Date().getFullYear();

export default function Footer() {
  return (
    <footer className="border-t border-hairline bg-bg">
      <div className="container-x grid gap-10 py-14 md:grid-cols-12">
        <div className="md:col-span-5">
          <div className="flex items-center gap-2 font-display text-xl font-bold">
            <span className="grid h-8 w-8 place-items-center rounded-md border border-hairline text-accent">
              A
            </span>
            Ariful<span className="text-accent">.</span>
          </div>
          <p className="mt-4 max-w-sm text-muted">
            ফ্রিল্যান্স ডিজিটাল মার্কেটার · ঢাকা, বাংলাদেশ। ব্র্যান্ড বাড়ানোর কাজে যেকোনো
            সময় নক দিন।
          </p>
        </div>

        <div className="md:col-span-3">
          <div className="eyebrow mb-4">নেভিগেশন</div>
          <ul className="space-y-2 text-muted">
            {[
              ["#home", "হোম"],
              ["#services", "সার্ভিস"],
              ["#work", "পোর্টফোলিও"],
              ["#about", "সম্পর্কে"],
              ["#contact", "যোগাযোগ"],
            ].map(([href, label]) => (
              <li key={href}>
                <a href={href} className="hover:text-text">
                  {label}
                </a>
              </li>
            ))}
          </ul>
        </div>

        <div className="md:col-span-4">
          <div className="eyebrow mb-4">যোগাযোগ</div>
          <ul className="space-y-2 text-muted">
            <li>
              <a
                href="https://wa.me/8801874783819"
                target="_blank"
                rel="noreferrer"
                className="hover:text-text"
              >
                WhatsApp — 01874783819
              </a>
            </li>
            <li>
              <a href="mailto:ariful68173@gmail.com" className="hover:text-text">
                ariful68173@gmail.com
              </a>
            </li>
          </ul>
          <div className="mt-5 flex items-center gap-2">
            {[
              { icon: Facebook, label: "Facebook" },
              { icon: Instagram, label: "Instagram" },
              { icon: Linkedin, label: "LinkedIn" },
              { icon: Youtube, label: "YouTube" },
            ].map(({ icon: Icon, label }) => (
              <a
                key={label}
                href="#"
                aria-label={label}
                className="grid h-9 w-9 place-items-center rounded-md border border-hairline text-muted transition hover:border-accent hover:text-accent"
              >
                <Icon size={15} />
              </a>
            ))}
          </div>
        </div>
      </div>

      <div className="border-t border-hairline">
        <div className="container-x flex flex-col items-center justify-between gap-3 py-5 sm:flex-row">
          <p className="font-mono text-[11px] uppercase tracking-widest2 text-muted">
            © {YEAR} Ariful Islam · সব অধিকার সংরক্ষিত
          </p>
          <p className="font-mono text-[11px] uppercase tracking-widest2 text-muted">
            The Ledger · v0.1
          </p>
        </div>
      </div>
    </footer>
  );
}
